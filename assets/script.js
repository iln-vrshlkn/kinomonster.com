...
const dots = document.querySelectorAll(".next-main-sider-dots>span");
const interval = 5;
...
const chackpoint = 600;
let nav_bg = 'transparent';
let opacity = 1;

window.addEventListener("scroll", () => {
    const currentScroll = window.pageYOffset;
    if (currentScroll <= chackpoint) {
        nav_bg = 'transparent';
        opacity = 1 - currentScroll / chackpoint;
    } else {
        nav_bg = '#000';
        opacity = 0;
    }
    ...
    slides.forEach(silde => silde.getElementsByTagName('img')[0].style.opacity = opacity);
});
